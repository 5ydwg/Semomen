...
class PostRepository {
  ...
  Future<List<PostModel>> getPopularPosts() async {
    ...
    List<QueryDocumentSnapshot<Object?>> posts = result.docs
        .where((element) => popularPostIds.contains(element.id))
        .toList();
  ...
  Future<List<PostModel>> getNewGuides() async {
    ...
    posts.sort((a, b) =>
        b.get('date_time').toString().compareTo(a.get('date_time').toString()));
    result = posts
        .map((e) => PostModel.fromDoc(e))
        .take(3)
        .toList(); // 가장 최신의 post를 3개만 가져오기
    return result;
  }
}
